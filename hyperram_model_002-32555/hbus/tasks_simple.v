...
begin:m
	...
	while(addr < count)begin
		if({ctrl.buffer[4+ctrl.latency+addr],ctrl.buffer[4+ctrl.latency+addr+1]} != ctrl.dut.top.Mem[address - 32'h400000] && {ctrl.buffer[0][4:0],ctrl.buffer[1],ctrl.buffer[2],ctrl.buffer[3],ctrl.buffer[5][2:0]} >= 32'h400000)begin
			$display("failed @ %x for data %x (%x)",address,{ctrl.buffer[4+ctrl.latency+addr],ctrl.buffer[4+ctrl.latency+addr+1]},ctrl.dut.top.Mem[address-32'h400000]);
			fail = fail+1;
		end else if({ctrl.buffer[4+ctrl.latency+addr],ctrl.buffer[4+ctrl.latency+addr+1]} != ctrl.dut.bottom.Mem[address])begin
			$display("failed @ %x for data %x (%x)",address,{ctrl.buffer[4+ctrl.latency+addr],ctrl.buffer[4+ctrl.latency+addr+1]},ctrl.dut.bottom.Mem[address]);
			fail = fail+1;
		end
		address = address+1;
