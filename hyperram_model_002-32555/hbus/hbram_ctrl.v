...
//    V1.0     VINI			     08 Apr 20    Initial release
///////////////////////////////////////////////////////////////////////////////
//
//  Module implement HyperBus controller for the test bench. 
//
///////////////////////////////////////////////////////////////////////////////
`timescale 1ps/1ps
...
`ifdef hram
		...
		if({buffer[1][3:0],buffer[2],buffer[3],buffer[5][2:0]} >= 32'h400000)
			rwds_sel <= dut.top.rwds_enable;
		else
			rwds_sel <= dut.bottom.rwds_enable;
	end
`elsif opi
	end else if(count == 6'h20 && !mem_write)begin
		if({buffer[2],buffer[3],buffer[4],buffer[5]} >= 32'h400000)
			rwds_sel <= dut.top.rwds_enable;
		else
			rwds_sel <= dut.bottom.rwds_enable;
	end
