...
//    V1.0     VINI			     17 Jan 21    Initial release
//    V2.0     VINI              14 Jun 23    updated RWDS logic to check refresh_pulse during CSNeg event. 
//	  V3.0	   HolubK			 02 Jul 25	  Fixed bit mask location
...
//                 512Mb high-speed 1.8 Volt Core, x8 data bus
//
//
//////////////////////////////////////////////////////////////////////////////
//  Comments :
...
//////////////////////////////////////////////////////////////////////////////
//  Known Bugs:
//      1. diferential clock is not used for functionality
//		2. a 1ps of delay on DQ, with respect to RWDS observed for lower frequency access. 
//		3. an additinal 1ps may require for tRP in simulation. 
//		4. Partial refresh uses a 64-bit variable to record the time. partial refresh simulation may not work properly if simulation time exceeds the 64bit value. 
//
//////////////////////////////////////////////////////////////////////////////
...
`define S80KS5122GABHI

`ifdef S80KS5122GABHI
...
module s80ks5122
	...
	hram_256mb  
		#(
			2'h1,				//die_id
			26'h400000,			//base address
			32'h1000000,			//id reg base address
			32'h1000800,			//config reg base address
			1'b1,				//variable/fixed latency. fixed to "FIXED LATENCY" for 512Mb HyperRam. VARIABLE LATENCY available only for 64Mb. ,
			"S80KS5122GAXXXXXX",//Timing Model
			"S80KS5122" 		//Part ID
		)
		top(
			.DQ7(DQ7)      ,
...
module hram_256mb 
	#(
	parameter die_id = 2'h0,
	parameter AddrRANGE_START = 26'h00000,
	parameter Identification_BASEADDRESS = 32'h00,
	parameter Config_BASEADDRESS = 32'h0800,
	parameter variable_latency = 1'b0,
	parameter TimingModel = "S80KS2562GAXXXXXX", //initialized to 256Mb hyperRam MPN, as this module denisity is 256Mb
    parameter PartID   = "S80KS5122"			
	)
    (
    DQ7      ,
    DQ6      ,
...
////////////////////////////////////////////////////////////////////////
	...
	reg softRESET_edge = 1'b0;
	reg softRESET      = 1'b0;
    wire soft_reset_wire = 1'b0;
    assign soft_reset_wire = softRESET;
    ...
    parameter HiAddrBit = 36;
	//parameter AddrRANGE_START = 25'h00000;
    parameter AddrRANGE_END = AddrRANGE_START + MemSize;

	localparam reg_wr_addr_mask_bit = 37;
	reg [44:0] reg_write_address_mask = ~(1 << reg_wr_addr_mask_bit);
...
///////////////////////////////////////////////////////////////////////////////
    ...
    $width (posedge soft_reset_wire          , tpw_RESETNeg_negedge);	 
...
///////////////////////////////////////////////////////////////////////////////
	...
	reg [15:0] Identification_reg0 = {die_id,14'h0E86};
    ...
    integer j;
        //assumptions:
        //1. TimingModel has format as S70KL1282XXXXXXXX
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                ALTERNATE_64  = 1'b0;
                HYBRID        = 1'b0;
		RWDSout_zd_tmp = Config_reg0[3] | refresh_pulse;
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    begin
						...
						if(refresh_pulse)begin
							...
							REFCOLL_ACTIV = 1'b0;//variable_latency;
							RWDSout_zd_tmp = 1'b0;
						end
					end
                end
                ...
                begin
					...
					if(Address >= AddrRANGE_START && Address <= AddrRANGE_END || Target)begin
						bus_cycle_state = DATA_BITS;
						if(!Target)begin
							Address = Address - AddrRANGE_START;
						end
					end else
        ...
        begin
                ...
                begin
                    ...
                    begin
                        ...
                        begin
							...
							if(ca_in[44:16] == Identification_BASEADDRESS[31:3] && ca_in[2:1] == Identification_BASEADDRESS[2:1])
							begin
								if(ca_in[0] == 1'b0)
								begin
							...
							end else if(ca_in[44:16] == Config_BASEADDRESS[31:3] && ca_in[2:1] == Config_BASEADDRESS[2:1])
							begin
								if(ca_in[0] == 1'b0)
								begin
            ...
            begin
                ...
                begin
                    Data_in[7:0] = Din;
                    data_cycle = data_cycle + 1;
					if (data_cycle == 2 && ((ca_in[44:0] & reg_write_address_mask) == (reg_write_address_mask & {Config_BASEADDRESS[31:3],13'h00,Config_BASEADDRESS[2:0]})))
                    //if (data_cycle == 2 && ca_in[44:16] == Config_BASEADDRESS[31:3] && ca_in[2:1] == Config_BASEADDRESS[2:1] && ca_in[0] == 1'b0)
                    begin
                        ...
                        Config_reg0=Data_in;
					end else if (data_cycle == 2 && ((ca_in[44:0] & reg_write_address_mask) == (reg_write_address_mask & {Config_BASEADDRESS[31:3],13'h00,Config_BASEADDRESS[2:1],1'b1})))
                    //end else if (data_cycle == 2 && ca_in[44:16] == Config_BASEADDRESS[31:3] && ca_in[2:1] == Config_BASEADDRESS[2:1] && ca_in[0] == 1'b1)
                    begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    if (Address == (AddrRANGE_END - AddrRANGE_START))
                                        Address = 0;
                                    else
                                        Address = Address + 1;
                                end
                                ...
                                begin
                                    ...
                                    begin
                                        ...
                                        (Start_BurstAddr/BurstLength)*BurstLength
                                                + BurstLength;
                                                if (Address==AddrRANGE_END - AddrRANGE_START + 1)
                                                    Address = 0;
                                                RD_MODE = CONTINUOUS;
                                            end
                                        end
        ...
        begin
            ...
            if (falling_edge_RESETNeg) begin
				...
				Identification_reg0 = {die_id,14'h0E86};//default value
 ...
 always @(posedge softRESET)
	 begin
		#(`tRP)softRESET_edge = softRESET;	
		#1 softRESET_edge = 1'b0;
	 end
 
 always @(posedge softRESET_edge or negedge CSNeg)
	begin 
		if (softRESET == 1'b1)	
			Config_reg1[15:12] = 4'hF;
		softRESET = 1'b0;
	end 
 
 always @(posedge CSNeg)
	begin 
		if (Config_reg1[15:12] == 4'b1010)
			begin 
				softRESET = 1'b0 ; //Disabling SoftRESET
			end 
	end
	
 always @(posedge RESETNeg or negedge softRESET)
    begin
        rising_edge_RESETNeg = 1;
        #1 rising_edge_RESETNeg = 0;
    end

 always @(negedge RESETNeg or posedge softRESET)
    begin
        #(`tRP) falling_edge_RESETNeg = (!RESETNeg) | softRESET;
 ...
 always @(falling_edge_CKDiff)
    ...
    BUFFERs_hram256Mb    BUF_DOut    (BuffOutDQ, BuffInDQ);
    BUFFERs_hram256Mb    BUF_RWDS    (BuffOutRWDS, BuffInRWDS);
    BUFFERs_hram256Mb    BUF_RWDSR   (BuffOutRWDSR, BuffInRWDSR);
...
module BUFFERs_hram256Mb (OUT,IN);
    input IN;
    output OUT;
    buf   ( OUT, IN);
endmodule
